...
class Causal_conv(Layer):
    ...
    def _call(self, inputs):
        ...
        inputs_conv1 = self.l_norm(inputs_conv1, name=self.name)
        inputs_conv1 = inputs_conv1 + inputs_res

        return self.act(inputs_conv1)


class Causal_conv_trans(Layer):
    ...
    def _call(self, inputs):
        ...
        if self.input_dim == self.output_dim:
            ...
            wt_res = tf.get_variable(name='wt_res', shape=[1, 1, self.input_dim, self.output_dim], dtype=tf.float32)
            inputs_res = tf.nn.conv2d(inputs, wt_res, strides=[1, 1, 1, 1], padding='VALID')
        ...
        inputs_conv1 = tf.nn.conv2d(inputs, wt1, strides=[1, 4, 1, 1], padding='VALID') + bt1
        inputs_conv1 = self.l_norm(inputs_conv1, name=self.name)
        print(inputs_res.shape)
        inputs_conv1 = inputs_conv1 + inputs_res[:, -self.n_pre:, :, :]
...
class Graph_base_conv(Layer):
    ...
    def _gconv(self, inputs, inputs_res, theta, bias):
        ...
        _, T, N, C = inputs.get_shape().as_list()
        node_en = tf.get_variable(name='embedding_1', shape=[N, 5], dtype=tf.float32)
